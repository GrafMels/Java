...
# Классы и методы (ООП)
    Вызов нестатического метода в статическом может осуществляться только через экземляр(prog) класса(Example006).

[Пример](Example006.java).

Различие класса, обьекта и экземпляра обьекта.

# Взлом модификатора private

    [Хакерство](https://t.me/iksergeyru/19).
